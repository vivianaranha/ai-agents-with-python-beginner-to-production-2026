# Tutorial 2: Job queues and task state

## Goal

Learn **job queues and task state** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **job queues and task state** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-16/tutorial-2
cd work/module-16/tutorial-2
```
...
Refactor the example so the program explicitly demonstrates **job queues and task state**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
