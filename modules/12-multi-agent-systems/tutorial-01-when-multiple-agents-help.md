# Tutorial 1: When multiple agents help

## Goal

Learn **when multiple agents help** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **when multiple agents help** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-12/tutorial-1
cd work/module-12/tutorial-1
```
...
Refactor the example so the program explicitly demonstrates **when multiple agents help**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
