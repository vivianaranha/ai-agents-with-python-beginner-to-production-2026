# Tutorial 4: Shared memory, handoffs and failure containment

## Goal

Learn **shared memory, handoffs and failure containment** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **shared memory, handoffs and failure containment** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-12/tutorial-4
cd work/module-12/tutorial-4
```
...
Refactor the example so the program explicitly demonstrates **shared memory, handoffs and failure containment**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
