# Tutorial 1: MCP architecture in 2026

## Goal

Learn **mcp architecture in 2026** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **mcp architecture in 2026** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-11/tutorial-1
cd work/module-11/tutorial-1
```
...
Refactor the example so the program explicitly demonstrates **mcp architecture in 2026**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
