# Tutorial 3: Container/cloud deployment concepts

## Goal

Learn **container/cloud deployment concepts** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **container/cloud deployment concepts** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-17/tutorial-3
cd work/module-17/tutorial-3
```
...
Refactor the example so the program explicitly demonstrates **container/cloud deployment concepts**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
