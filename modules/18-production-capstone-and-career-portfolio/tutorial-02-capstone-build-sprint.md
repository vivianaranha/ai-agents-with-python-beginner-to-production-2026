# Tutorial 2: Capstone build sprint

## Goal

Learn **capstone build sprint** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **capstone build sprint** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-18/tutorial-2
cd work/module-18/tutorial-2
```
...
Refactor the example so the program explicitly demonstrates **capstone build sprint**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
