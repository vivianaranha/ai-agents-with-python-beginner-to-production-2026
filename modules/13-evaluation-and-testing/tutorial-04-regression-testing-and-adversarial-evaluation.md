# Tutorial 4: Regression testing and adversarial evaluation

## Goal

Learn **regression testing and adversarial evaluation** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **regression testing and adversarial evaluation** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-13/tutorial-4
cd work/module-13/tutorial-4
```
...
Refactor the example so the program explicitly demonstrates **regression testing and adversarial evaluation**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
