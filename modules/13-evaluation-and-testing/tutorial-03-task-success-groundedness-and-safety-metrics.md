# Tutorial 3: Task success, groundedness and safety metrics

## Goal

Learn **task success, groundedness and safety metrics** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **task success, groundedness and safety metrics** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-13/tutorial-3
cd work/module-13/tutorial-3
```
...
Refactor the example so the program explicitly demonstrates **task success, groundedness and safety metrics**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
