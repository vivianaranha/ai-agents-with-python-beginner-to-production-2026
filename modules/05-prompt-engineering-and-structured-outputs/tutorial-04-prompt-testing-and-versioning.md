# Tutorial 4: Prompt testing and versioning

## Goal

Learn **prompt testing and versioning** and apply it to a small part of a support-operations assistant.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **prompt testing and versioning** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-05/tutorial-4
cd work/module-05/tutorial-4
```
...
Refactor the example so the program explicitly demonstrates **prompt testing and versioning**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
