# Tutorial 4: Threat modeling, red teaming and safety controls

## Goal

Learn **threat modeling, red teaming and safety controls** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **threat modeling, red teaming and safety controls** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-14/tutorial-4
cd work/module-14/tutorial-4
```
...
Refactor the example so the program explicitly demonstrates **threat modeling, red teaming and safety controls**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
