# Tutorial 2: Tool abuse, data leakage and least privilege

## Goal

Learn **tool abuse, data leakage and least privilege** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **tool abuse, data leakage and least privilege** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-14/tutorial-2
cd work/module-14/tutorial-2
```
...
Refactor the example so the program explicitly demonstrates **tool abuse, data leakage and least privilege**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
