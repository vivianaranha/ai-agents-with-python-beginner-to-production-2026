# Tutorial 1: What makes a tool safe and useful

## Goal

Learn **what makes a tool safe and useful** and apply it to a small part of a support-operations assistant.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **what makes a tool safe and useful** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-06/tutorial-1
cd work/module-06/tutorial-1
```
...
Refactor the example so the program explicitly demonstrates **what makes a tool safe and useful**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
