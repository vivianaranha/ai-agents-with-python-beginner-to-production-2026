# Tutorial 2: Expose an agent as an API

## Goal

Learn **expose an agent as an api** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **expose an agent as an api** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-15/tutorial-2
cd work/module-15/tutorial-2
```
...
Refactor the example so the program explicitly demonstrates **expose an agent as an api**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
