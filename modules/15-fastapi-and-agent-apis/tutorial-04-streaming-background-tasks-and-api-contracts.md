# Tutorial 4: Streaming, background tasks and API contracts

## Goal

Learn **streaming, background tasks and api contracts** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **streaming, background tasks and api contracts** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-15/tutorial-4
cd work/module-15/tutorial-4
```
...
Refactor the example so the program explicitly demonstrates **streaming, background tasks and api contracts**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
