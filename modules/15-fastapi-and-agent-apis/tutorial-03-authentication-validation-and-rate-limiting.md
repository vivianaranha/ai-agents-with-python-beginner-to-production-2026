# Tutorial 3: Authentication, validation and rate limiting

## Goal

Learn **authentication, validation and rate limiting** and apply it to a small part of a enterprise operations agent.

## Why this matters for agents

AI agents combine probabilistic model behavior with deterministic software. The deterministic layer must validate data, control side effects, persist state, enforce permissions and recover from failure. This tutorial treats **authentication, validation and rate limiting** as an engineering building block rather than an isolated Python topic.
...
```bash
mkdir -p work/module-15/tutorial-3
cd work/module-15/tutorial-3
```
...
Refactor the example so the program explicitly demonstrates **authentication, validation and rate limiting**. Keep each function small. Give every function a clear input and output. Avoid hidden global state.
